...
export class Valence {
    ...
    public listen(port?: ElectronPorts, callback?: (port?: ElectronPorts) => void): void;
    public listen(
    ...
    private async execute(request: RequestObject, port: MessagePortMain): Promise<void> {
        const pipeline = this.routesMap.get(request.route);

        if (pipeline === undefined) {
            throw new ReferenceError("Route was not found. Check if a specified route exists.");
        }
    ...
    private contextBuilder(request: ValenceRequest, port: MessagePortMain): ValenceContext {
