...
async function attachIpcHandlers(): Promise<void> {
    ...
    ipcMain.handle("getAllPasswords", handleInvokeRequest);
    ipcMain.handle("savePassword", handleInvokeRequest);
}

async function handleInvokeRequest<T extends RequestObject, K extends ResponseObject>(
...
function forkUtilityProcess(scriptPath: string): UtilityProcess {
    ...
    child.once("spawn", () => {
        ...
        console.log(
            `[Child process][Spawn Event fired] Forking utility process ${child.pid} at ${path.join(
                __dirname,
                scriptPath
